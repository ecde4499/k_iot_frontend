...
//? Library 클래스: Book 객체 목록 관리 & 추가 기는 구현
class Library {
  constructor() {
    this.books = []; // 초기에는 책 없음! (빈 배열)
    this.nextBookId = 1; // 책 교유 값
  }
  ...
  updateBook(id, newTitle, newAuthor) {
    // +) find 메서드는 배열에서 제공된 콜백함수를 만족하는 첫 번째 주소를 반환
    ...
    // 등록된 책 - 수정
    // 논리연산자) null과 undefined 검증 && 빈 빈문자열 검증
    ...
    console.log(
      `책 (ID: ${book.id}) 정보가 업데이트 되었습니다. : 제목 - ${book.title}, 저자 - ${book.author}`
    );
  }
  ...
  removeBook(id) {
    ...
    if (idx !== -1) {
      ...
      console.log(
        `${removedBook.title} (ID: ${removedBook.id}) 책이 도서관에서 삭제되었습니다`
      );
    } else {
      console.log("해당 책을 찾을 수 없습니다.");
    }

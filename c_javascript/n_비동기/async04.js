...
// await(기다리다)
// - async 키워드로 정의된 함수 내에서 swait 키워드를 사용
...
async function fetchCustomerData() {
  try {
    ...
    if (!data.ok) {
      throw new Error('네트워크 응답이 실패하여 데이터를 정산적으로 가져오지 못했습니다.');
    }

    // .json(): json 데이터를 parse분석하여 JS객체로 반환
...
// fetch() 함수
// : 네트워크 요청을 비동기적으로 처리
// - HTTP 응답을 나타내는 Response 객체를 반환
// +) 성공 시 resolve 값이 Response 객체
// +) 실패 시 reject 값에 예외가 담김 - 네트웨크 자체가 실패했을 때만 발생!
...
//4) haders: 응답의 헤더를 나타냄, 응답의 메타데이터나 특정 값을 포함

...
document.addEventListener("DOMContentLoaded", () => {
  ...
  //& 2. 시작 시 잠시동안 모든 카드의 뒷면(색성)을 공개하는 함수 정의
  ...
  //& 9. 버큰 가시성 토글 함수
  function toggleButtonVisibillty(isGameStarted) {
  ...
  startButton.addEventListener('click', () => {
    ...
    // 버튼의 가지성 조정: 시작 X, 재시작 O, 완료 O
    toggleButtonVisibillty(true);
  ...
  completedButton.addEventListener('click', () => {
    ...
    // - 배열의 모든 요소가 주어진 함수 조건식을 만족할 때 (true값일 경우) ture를 반환
    ...
    if (allflipped) {
      ...
      toggleButtonVisibillty(false); // 시작 버튼만 보이도록
    } else {
      alert('완료되지 않았습니다.');
    }
  });

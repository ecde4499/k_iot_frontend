...
//& 2. 비동기 작업을 사용하여 JSONPlaceholder에서 사용자 데이터를 가져오는 함수 정의
async function fetchUserData(userId: number) {
  try {
    ...
    const data = await response.json(); // 객체 형식으로 데이터 변환
    ...
    if (data.id) {
      const userData: FetchResponse = {
        ...
        company: {
          ...
          catachPhrase: data.company.catachPhrase
        }
      };
...
fetchUserData(1);
// Name: Ervin Howell, Email: Shanna@melissa.tv, Company: Deckow-Crist
fetchUserData(2);

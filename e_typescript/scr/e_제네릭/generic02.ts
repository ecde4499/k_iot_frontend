...
function printLength<T>(arg: T): void {
  // console.log(arg.length); // T 형식에 length 속성이 없음
  // >> T는 반드시 length속성
}
...
// T의 타입 변수가 반스디! ILength 인터페이스를 포함하는 타입
...
// constraints<number>(123); // 'number'형식이 ILength 제약조건을 만족 X
...
function checkType<T>(value: T): Check<T> {
  let result = typeof value === 'string' ? 
    ('String' as Check<T>) : ('Not a String' as Check<T>);
  return result;
}

...
const loadPage = async (page: number) => {
  ...
  // 다음 페이지 존재 여부는 현대 응답 개수로 판단
  isLastPage = users.length < limit;
  updateButtons();
}

const updateButtons = () => {
  const prevPageButton = document.getElementById('prev-button') as HTMLButtonElement;
  const nextPageButton = document.getElementById('next-button') as HTMLButtonElement;

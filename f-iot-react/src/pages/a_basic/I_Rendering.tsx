...
function Item({ name, isPacked }: ItemType) {
  ...
  //? react의 JSX 괄호가 문법적 요소로 사용 - return문 위에서 작성
  // if (isPacked) {
  //   return (
  //     <li>{name} ✔️</li>
  //   )
...
function I_Rendering() {
  ...
  // : 요소 개수 변화 O
  // - 조건에 따른 렌더링
  const businessPeople = peopleInfo.filter(person => person.job === '강사');
  ...
  // - map()을 통해 여러 요소를 렌더링 할 때 key는 React가 어떤 항목이
  //    변경, 추가 또는 삭제 되었는지 파악하는 용도로 사용
  //? 같읕 배열 내에서 유일 (데이터의 PK값을 주로 사용)
  return (
    <div>
      <p>여행용 짐 목록</p>

...
const longinInitialValue: Login = {
  id: "",
  password: "",
};
...
function State02() {
  ...
  const [login, setLogin] = useState<Login>(longinInitialValue);
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    // 데이터에 대한 활용(제출, 사용) gndpsms chrlghkrk vlftn
    setLogin(longinInitialValue);
  }
  ...
  const handleLoginChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLogin({
      ...
      // 변경하고자하는 name 키를 가진 value 값을 변경 (해당 필드만 값 업데이트)
      [name]: value,
    });
  };

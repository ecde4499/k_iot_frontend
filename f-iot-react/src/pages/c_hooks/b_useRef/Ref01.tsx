...
function Ref01() {
  ...
  //% 13) 'a'의 길이값 1 유지
  const lengthRef = useRef<number>(0) //# 2) useRef 호출 시 { currunt: 0 } 객체 생성 & 저장

  // 2) 일반 변수
  //    - 함수형 컴포넌트는 변화를 감지하면 렌더링 될 때마다 함수 전체가 다시 실행!
  //    - 아래의 number 변수에 매번 새로운 초기화가 진행
  //% 14) 컴포넌트의 재실행으로 변수 새롭게 선언되고 다시 e으로 초기화
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // : 일반 JS 객체 속성 변경과 동일
    // - 렌더링 필요 없는 대상 (상태 관리와 무관, ref는 화면 갱신 대상 X)
  ...
  return (
    <div>
      ...
      */}
      {/* //% 16) 값의 유지 X: 0 출력 */}
      <p>컴포넌트가 렌더링 될 때 마다 초기화되는 값: {lengthVar}</p>
    </div>
  )
}

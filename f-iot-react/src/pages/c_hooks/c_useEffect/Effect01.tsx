...
? useEffect는 반환값 없이 사용
  useEffect(() => {
    ...
    ? useEffect의 정리 함수
      >> useEffect에서 함수를 return하면 해당 함수는 컴포넌트 화면에서 사라질 때 (언마운트) 실행
...
function Effect01() {
  ...
  //& 의존성 배열이 없는 useEffect: 모든 랜더링 시에 동작
  useEffect(() => {
  ...
  // : 마운트 시 + name + count 값 상태 변경
  useEffect(() => {
    console.log(`count, name값을 의존성 배열로 가지는 useEffect`);
  }, [count, name]);

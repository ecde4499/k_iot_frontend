...
//! 1. 예약 데이터 타입 정의
type ReservationStatus = "PENDING" | "CONFIRMED" | "CANCLELLED";

type Reservation = {
  id: number;                   // 예약 고유 Id
  customer_name: string;        // 고객 이름
  truck_name: string;           // 예약한 푸드트럭 일흠
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP 에러! 상새코드: ${response.status}`);
      }

      const data = await response.json(); // JSON 파싱

      // 실제 예약 데이터 구조로 변환 (임시 매핑)
      // > slice(시작 인덱스, 끝 인덱스)까지의 얕은 복사본을 새로운 배열로 반환 > 끝 인덱스 미포함
      const mappedData: Reservation[] = data.slice(0, 5).map((item: any, idx: number) => ({
        ...
        status: idx % 2 === 0 ? 'PENDING' : 'COMFIRMED' // 홀짝에 따라 상태 다르게
      }));
        ...
        {reservations.map(reservation => (
          <li key={reservation.id} style={{ 
            ...
            <div style={{ marginTop: '8px' }}>
              {/* COMFIRMED가 아닐 때만 승인 버튼 */}
              {reservation.status !== 'CONFIRMED' && (
                <button onClick={() => updateReservationStatus(reservation.id, "CONFIRMED")}> 승인</button>
              )}

              {/* CANCELLED가 아닐 때만 취소 버튼 */}
              {reservation.status !== 'CANCLELLED' && (
                <button onClick={() => updateReservationStatus(reservation.id, "CANCLELLED")}>취소</button>
              )}
            </div>
          </li>
        ))}
